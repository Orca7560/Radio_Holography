#!/usr/bin/env python3
"""
group_up_txt_prd.py — fringe/frinZ結果(*.txt)を1つのbeamファイルに統合する。

概要:
  `<input>/fringe_results/` （--band-split指定時は `fringe_results_<band>/`）
  以下の複数の.txtファイル（fringe/frinZの出力を1行1積分でまとめたもの）を
  時刻順に連結し、カンマ区切りの `beam.txt`（既定名）にまとめる。
  さらに、PRD（アンテナ指令位置ログ）またはSKDスケジュールを与えることで、
  各積分の中央時刻におけるAz/Elオフセット（本走査時は駆動速度も）を付加できる。
...
主なオプション:
  ...
  --maser            SNRの代わりにFrequency列を取得する（メーザー観測向け）。
  --band-split N     8192-8704MHzの帯域を512MHzのN分割（Nは512の約数）で
                     処理し、`fringe_results_<band>/` ごとに
                     `beam_<band>.txt` を出力する。このとき --output は
                     指定できない。

出力される列（左から）:
  Epoch, Length, Amp, Phase, [Frequency または SNR], [Res-Delay(--add-delay時)],
  [Az_Offset, El_Offset, Az_Rate_arcmin_s, El_Rate_arcmin_s (--prd使用時)]
  または [Az_Offset, El_Offset (--skdのみ使用時、現状CLIからは未接続)]

実行例:
  python group_up_txt_prd.py --input I26184Y
  python group_up_txt_prd.py --input I26184Y --prd I26184Y/I26184Y32.prd \\
      --scan-lag-ms 0 --output I26184Y/beam.txt
  python group_up_txt_prd.py --input I26184Y --maser --add-delay
  python group_up_txt_prd.py --input I26184Y --band-split 8
"""
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description=(
            "fringe_results/ 以下のfringe/frinZ結果txtを1つのbeamファイルに"
            "統合し、必要に応じてPRD/SKDのAz/Elオフセットを付加します。"
        ),
        epilog=(
            "実行例:\n"
            "  python group_up_txt_prd.py --input I26184Y\n"
            "  python group_up_txt_prd.py --input I26184Y "
            "--prd I26184Y/I26184Y32.prd --scan-lag-ms 0\n"
            "  python group_up_txt_prd.py --input I26184Y --maser --add-delay\n"
            "  python group_up_txt_prd.py --input I26184Y --band-split 8\n\n"
            "--prd を省略すると --input 配下から *<antenna>.prd を自動検出します"
            "（1つに絞れない場合はエラーになるため --prd を明示してください）。\n"
            "--band-split 使用時は --output を指定せず、beam_<band>.txt が"
            "帯域ごとに出力されます。"
        ),
