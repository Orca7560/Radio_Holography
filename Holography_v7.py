...
SKD_FILE  = os.path.join(BASE_DIR, "schedule_scaning.skd")
BEAM_FILE = os.path.join(BASE_DIR, "beam_100_I26184Y_hosei.txt")
SKD_FILE  = os.path.join(BASE_DIR, "schedule_100.skd")
...
if AUTO_TIME_ALIGN:
    ...
    sked_center_mask = (sked_r2 < 0.1) & (np.append(np.diff(sked_time), 0) > 1.0) 
    ...
    t_common = np.arange(max(beam_time.min(), sked_time_for_align.min()), min(beam_time.max(), sked_time_for_align.max()), 0.1)
    ...
    if len(t_common) > 10:
        ...
        correlation = np.correlate(beam_amp_interp - beam_amp_interp.mean(), sked_model_interp - sked_model_interp.mean(), mode='full')
        lags = np.arange(-len(t_common) + 1, len(t_common)) * 0.1
        time_offset = lags[np.argmax(correlation)]
        print(f"-> 自動推定されたタイムラグ: {time_offset:+.2f} 秒")
