...
int main(int argc,char**argv){
 try{
  ...
  std::vector<S>a;while(std::getline(f,l)){auto v=split(l);if((int)v.size()<(int)hd.size())continue;try{a.push_back({epoch(v[col["Epoch"]]),std::stod(v[col["Amp"]]),std::stod(v[col["Phase"]]),std::stod(v[col["SNR"]])});}catch(...){}}
