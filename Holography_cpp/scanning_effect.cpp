...
int main(int argc,char**argv){
 try{
  ...
  fs::path out=arg(argc,argv,"--output","--out","scanning_result");fs::create_directories(out);
  ...
  std::vector<S>a;while(std::getline(f,l)){auto v=split(l);if((int)v.size()<(int)hd.size())continue;try{a.push_back({epoch(v[col["Epoch"]]),std::stod(v[col["Amp"]]),std::stod(v[col["Phase"]]),std::stod(v[col["SNR"]]),0.0,0.0,0.0,0.0,0});}catch(...){}}
  ...
  for(auto&x:a){auto it=std::lower_bound(knots.begin(),knots.end(),x.t,[](auto&k,double t){return k[0]<t;});if(it==knots.begin()||it==knots.end())continue;auto p=it-1;double u=(x.t-(*p)[0])/((*it)[0]-(*p)[0]);x.az=(*p)[1]*(1-u)+(*it)[1]*u;x.el=(*p)[2]*(1-u)+(*it)[2]*u;x.vx=((*it)[1]-(*p)[1])/((*it)[0]-(*p)[0]);x.vy=((*it)[2]-(*p)[2])/((*it)[0]-(*p)[0]);x.dir=(x.vx>1e-5)-(x.vx<-1e-5);}
  ...
  std::ofstream o(out/"matched_and_corrected.csv");o<<"Epoch,Amp,Phase,SNR,Az_Offset,El_Offset,Az_Corrected,El_Corrected\n";for(auto&x:a)o<<x.t<<","<<x.amp<<","<<x.phase<<","<<x.snr<<","<<x.az<<","<<x.el<<","<<x.az-x.vx*best/1000<<","<<x.el-x.vy*best/1000<<"\n";
  std::ofstream q(out/"lag_search.csv");q<<"best_lag_ms,mismatch\n"<<best<<","<<bscore<<"\n";std::cout<<"Best scan lag: "<<best<<" ms\nOutputs: "<<out<<"\n";return 0;
